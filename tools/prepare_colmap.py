...
parser.add_argument("--base_path", type=str, required=True,
                    help="Path to the COLMAP output folder, containing cameras.txt, images.txt, points3D.txt")
...
if test_image_list_path is not None:
    ...
    df["is_train"] = df.index != 500
    df["is_train"][8] = False
    df["is_train"][10] = False
    df["is_train"][14] = False
    df["is_train"][20] = False
    df["is_train"][22] = False

# test_images = [f"00{idx}.png" for idx in range(175, 250)]
# select training data and validation data, have a val every 3 frames
train_df_all = df.copy()
...
train_df_all.drop(columns=["is_train"], inplace=True)
...
train_df_all.to_json(os.path.join(
    output_dir, "train_all.json"), orient="records")
