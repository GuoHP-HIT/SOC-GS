...
# OVER_RECONSTRUCTION_SCALE = RGB_FX_FY / MS_FX_FY
...
class GaussianPointAdaptiveController:
    ...
    class GaussianPointAdaptiveControllerConfig(YAMLWizard):
        ...
        num_iterations_reset_alpha: int = 300000
    ...
    def update(self, input_data: GaussianPointCloudRasterisation.BackwardValidPointHookInput):
        if input_data.hook_modality == 'RGB':
            ...
            with torch.no_grad():
                ...
                self.accumulated_position_gradients[
                    input_data.point_id_in_camera_list] += input_data.grad_point_in_camera
                self.accumulated_position_gradients_norm[
            ...
            with torch.no_grad():
                ...
                self.accumulated_view_space_position_gradients_ms[
                    input_data.point_id_in_camera_list] += grad_viewspace_norm_ms
                ...
                self.accumulated_view_space_position_gradients_avg_ms[
                    input_data.point_id_in_camera_list] += avg_grad_viewspace_norm_ms
                self.accumulated_position_gradients_ms[
                    input_data.point_id_in_camera_list] += input_data.grad_point_in_camera
                self.accumulated_position_gradients_norm_ms[
                    input_data.point_id_in_camera_list] += input_data.grad_point_in_camera.norm(dim=1)
        ...
        if self.iteration_counter < self.config.num_iterations_warm_up:
            ...
            if self.iteration_counter_ms % self.config.num_iterations_densify != 0:
                self._find_densify_points(input_data)
                self.input_data = input_data
            elif self.iteration_counter_ms % self.config.num_iterations_densify == 0:
                if self.iteration_counter_ms == 0:
                    pass
                else:
    ...
    def refinement(self):
        with torch.no_grad():
            ...
            if self.iteration_counter <= self.config.warmup_single_modality_iterations and self.iteration_counter % self.config.num_iterations_densify == 0:
                ...
                self.accumulated_position_gradients_norm = \
                    torch.zeros_like(self.maintained_parameters.pointcloud[:, 0], dtype=torch.float32)
            if self.iteration_counter > self.config.warmup_single_modality_iterations and self.iteration_counter % self.config.num_iterations_densify == 0 and self.iteration_counter_ms % self.config.num_iterations_densify == 0:
    ...
    def _find_densify_points_ms(self, input_data: GaussianPointCloudRasterisation.BackwardValidPointHookInput):
        ...
        point_depth_in_camera: torch.Tensor = input_data.point_depth
        point_uv_in_camera: torch.Tensor = input_data.point_uv_in_camera
        average_num_affect_pixels_ms = self.accumulated_num_pixels_ms / self.accumulated_num_in_camera_ms
        average_num_affect_pixels_ms[torch.isnan(average_num_affect_pixels_ms)] = 0
        ...
        multi_frame_average_accumulated_avg_pixel_view_space_position_gradients[
            torch.isnan(multi_frame_average_accumulated_avg_pixel_view_space_position_gradients)] = 0
        multi_frame_densify_mask |= (
                multi_frame_average_accumulated_avg_pixel_view_space_position_gradients / average_num_affect_pixels_ms > self.config.densification_multi_frame_view_pixel_avg_space_position_gradients_threshold)
    ...
    def _add_densify_points_cross_spectral(self):
        assert self.densify_point_info is not None and self.densify_point_info_ms is not None
        ...
        invalid_point_id_to_fill_sum = torch.where(self.maintained_parameters.point_invalid_mask == 1)[0][
                                       :num_of_densify_points]
        invalid_point_id_to_fill_RGB = invalid_point_id_to_fill_sum[:num_of_densify_points_RGB]
        invalid_point_id_to_fill_MS = invalid_point_id_to_fill_sum[num_of_densify_points_RGB:]
        ...
        if num_of_densify_points > 0:
            ...
            # set MS color separately
            # self.maintained_parameters.pointcloud_features[invalid_point_id_to_fill_RGB, :56] = \
            #     self.maintained_parameters.pointcloud_features[
            #     self.densify_point_info.densify_point_id[:num_of_densify_points_RGB], :56]
            # self.maintained_parameters.pointcloud_features[invalid_point_id_to_fill_RGB, 56:] = 0.
            ...
            # set RGB color separately
            # self.maintained_parameters.pointcloud_features[invalid_point_id_to_fill_MS, :8] = \
            #     self.maintained_parameters.pointcloud_features[
            #     self.densify_point_info_ms.densify_point_id[:num_of_densify_points_MS], :8]
            # self.maintained_parameters.pointcloud_features[invalid_point_id_to_fill_MS, 8:56] = 0.
            # self.maintained_parameters.pointcloud_features[invalid_point_id_to_fill_MS, 56:] = \
            #     self.maintained_parameters.pointcloud_features[
            #     self.densify_point_info_ms.densify_point_id[:num_of_densify_points_MS], 56:]
            ...
            print(
                f"<RGB rasterisation densify> num_over_reconstructed: {num_over_reconstructed_RGB}, num_under_reconstructed: {num_under_reconstructed_RGB}")
            print(
                f"<MS rasterisation densify> num_over_reconstructed: {num_over_reconstructed_MS}, num_under_reconstructed: {num_under_reconstructed_MS}")
            ...
            print(
                f"total valid points: {total_valid_points_before_densify} -> {total_valid_points_after_densify}, num_densify_points: {num_of_densify_points}")
            print(
                f"<RGB rasterisation densify> num_transparent_points: {num_transparent_points_RGB}, num_floaters_points: {num_floaters_points_RGB}, num_fillable_densify_points: {num_of_densify_points_RGB}")
            print(
                f"<MS rasterisation densify> num_transparent_points: {num_transparent_points_MS}, num_floaters_points: {num_floaters_points_MS}, num_fillable_densify_points: {num_of_densify_points_MS}")
