...
vec4f = ti.types.vector(n=4, dtype=ti.f32)
...
def generate_point_attributes_in_camera_plane_all(
    ...
    for idx in range(point_id_list.shape[0]):
        ...
        point_color[idx, 0], point_color[idx, 1], point_color[idx, 2],  point_color[idx, 3]= \
...
def gaussian_point_rasterisation_all(
    ...
    for pixel_offset in ti.ndrange(camera_height * camera_width):  # 1920*1080
        ...
        T_i = 1.0
        # accumulated_color = ti.math.vec3([0., 0., 0.])
        accumulated_color = vec4f([0., 0., 0., 0.])
        accumulated_depth = 0.
        depth_normalization_factor = 0.
        offset_of_last_effective_point = start_offset
        valid_point_count: ti.i32 = 0
        ...
        for point_group_id in range(num_point_groups):
            ...
            for point_group_offset in range(max_point_group_offset):
                ...
                # color = ti.math.vec3([tile_point_color[0, point_group_offset], tile_point_color[1, point_group_offset],
                #                       tile_point_color[2, point_group_offset]])
                color = vec4f([tile_point_color[0, point_group_offset], tile_point_color[1, point_group_offset],
...
@ti.kernel
def gaussian_point_rasterisation_multispectral_backward(
        camera_height: ti.i32,
        camera_width: ti.i32,
        camera_intrinsics: ti.types.ndarray(ti.f32, ndim=2),  # (3, 3)
        pointcloud: ti.types.ndarray(ti.f32, ndim=2),  # (N, 3)
        ...
        in_camera_grad_t_buffer: ti.types.ndarray(ti.f32, ndim=1),  # (M, 4)
        in_camera_grad_q_buffer: ti.types.ndarray(ti.f32, ndim=1),  # (M, 4)
        point_uv: ti.types.ndarray(ti.f32, ndim=2),  # (M, 2)
        point_in_camera: ti.types.ndarray(ti.f32, ndim=2),  # (M, 3)
    ...
    for pixel_offset in ti.ndrange(camera_height * camera_width):
        ...
        pixel_ms_grad = rasterized_image_grad[pixel_v, pixel_u, 3]
        ...
        for point_block_id in range(num_point_blocks):
            ...
            for inverse_point_offset_offset in range(max_inverse_point_offset_offset):
                ...
                if prod_alpha >= 1. / 255.:
                    ...
                    #     tile_point_color[3, idx_point_offset_with_sort_key_in_block],
                    # ])
                    color = tile_point_color[3, idx_point_offset_with_sort_key_in_block]
                    ...
                    point_grad_t_pointcloud_camera = d_pixel_rgb_d_color * pixel_ms_grad * \
                                                     (
                                                                 1. - point_alpha_after_activation_value) * point_alpha_after_activation_value
                    # print(d_pixel_rgb_d_color)
                    # \frac{dC}{da_i} = c_i T(i) - \frac{1}{1 - a_i} w_i
                    ...
                    # ti.atomic_add(
                    #     in_camera_grad_color_buffer[point_offset, 3], point_grad_color[3])
                    ti.atomic_add(
                        in_camera_grad_color_buffer[point_offset, 3], point_grad_color)
                    ti.atomic_add(
                        in_camera_grad_t_buffer[point_offset,], point_grad_t_pointcloud_camera
                    )
                    ti.atomic_add(
    ...
    for idx in range(point_id_in_camera_list.shape[0]):
        ...
        # point_grad_color = ti.math.vec4(
        #     in_camera_grad_color_buffer[idx, 0],
        #     in_camera_grad_color_buffer[idx, 1],
        #     in_camera_grad_color_buffer[idx, 2],
        #     in_camera_grad_color_buffer[idx, 3],
        # )
        point_grad_color = in_camera_grad_color_buffer[idx, 3]
        point_grad_t = in_camera_grad_t_buffer[idx]
        point_q_camera_pointcloud = ti.Vector(
            [q_camera_pointcloud[point_object_id[point_id], idx] for idx in ti.static(range(4))])
        point_t_camera_pointcloud = ti.Vector(
            [t_camera_pointcloud[point_object_id[point_id], idx] for idx in ti.static(range(3))])
        ...
        # calculate d_Sigma'/d_quat
        d_Sigma_prime_d_q_point_cloud_camera = gaussian_point_3d.project_to_camera_position_covariance_jacobian(
            T_camera_world=T_camera_pointcloud_mat,
            projective_transform=camera_intrinsics_mat,
            translation_camera=translation_camera,
        )
        ...
        ray_direction = gaussian_point_3d.translation - ray_origin
        ms_jacobian, direction_jacobian = gaussian_point_3d.get_color_with_direction_jacobian_by_ray(
            ray_origin=ray_origin,
            ray_direction=ray_direction,
        )
        ...
        # color_ms_grad = point_grad_color[3] * ms_jacobian
        color_ms_grad = point_grad_color * ms_jacobian
        t_ms_grad = point_grad_t * direction_jacobian * (-1.0)
        # print(direction_jacobian)
        for idx in ti.static(range(3)):
            ti.atomic_add(grad_t_pointcloud_camera_buffer[idx], t_ms_grad[idx])
        ...
        point_camera_q_grad = point_grad_uv_cov_flat @ d_Sigma_prime_d_q_point_cloud_camera
        for idx in ti.static(range(4)):
            ti.atomic_add(grad_q_pointcloud_camera_buffer[idx], point_camera_q_grad[idx])
    ...
    for i in ti.static(range(3)):
        grad_t_pointcloud_camera[0, i] = grad_t_pointcloud_camera_buffer[i]
    for i in ti.static(range(4)):
        grad_q_pointcloud_camera[0, i] = grad_q_pointcloud_camera_buffer[i]


@ti.kernel
def gaussian_point_rasterisation_pose_only_backward(
        camera_height: ti.i32,
        camera_width: ti.i32,
        camera_intrinsics: ti.types.ndarray(ti.f32, ndim=2),  # (3, 3)
        pointcloud: ti.types.ndarray(ti.f32, ndim=2),  # (N, 3)
    ...
    camera_intrinsics_mat = ti.Matrix(
        [[camera_intrinsics[row, col] for col in ti.static(range(3))] for row in ti.static(range(3))])
    qx, qy, qz, qw = q_pointcloud_camera[0, 0], q_pointcloud_camera[0, 1], q_pointcloud_camera[0, 2], \
                     q_pointcloud_camera[0, 3]
    q_norm_sqrt = qx ** 2 + qy ** 2 + qz ** 2 + qw ** 2
    inverse_jacobian = ti.math.mat3([
        [(qx ** 2 - qy ** 2 - qz ** 2 + qw ** 2) / q_norm_sqrt, (2 * qx * qy + 2 * qz * qw) / q_norm_sqrt,
         (2 * qx * qz - 2 * qy * qw) / q_norm_sqrt],
        [(2 * qx * qy - 2 * qz * qw) / q_norm_sqrt, (-qx ** 2 + qy ** 2 - qz ** 2 + qw ** 2) / q_norm_sqrt,
         (2 * qx * qw + 2 * qy * qz) / q_norm_sqrt],
        [(2 * qx * qz + 2 * qy * qw) / q_norm_sqrt, (-2 * qx * qw + 2 * qy * qz) / q_norm_sqrt,
         (-qx ** 2 - qy ** 2 + qz ** 2 + qw ** 2) / q_norm_sqrt],
    ])
    ti.loop_config(block_dim=(TILE_HEIGHT * TILE_WIDTH))
    ...
    for pixel_offset in ti.ndrange(camera_height * camera_width):
        ...
        # \frac{dC}{da_i} = c_i T(i) - \frac{1}{1 - a_i} w_i
        w_i = ti.math.vec3(0.0, 0.0, 0.0)
        # we use accumulated_multiplication to calculate d_T(i)_d_q
        accumulated_muliplication = 1.0
        first_loop_flag = 0.0
        ...
        for point_block_id in range(num_point_blocks):
            ...
            if to_load_idx_point_offset_with_sort_key >= block_start_idx_point_offset_with_sort_key:
                ...
                for i in ti.static(range(4)):
                    tile_point_uv_conic_and_rescale[i,
                                                    thread_id] = point_uv_conic_and_rescale[to_load_point_offset, i]
                # actually one 4 channels image will be obtained every rendering, here we just need to calculate
                # the rendered RGB image so tile_point_color's shape is [3, 1]
                for i in ti.static(range(3)):
                    tile_point_color[i,
                                     thread_id] = point_color[to_load_point_offset, i]
            ...
            for inverse_point_offset_offset in range(max_inverse_point_offset_offset):
                ...
                if prod_alpha >= 1. / 255.:
                    ...
                    # # add Ti gradient
                    # gaussian_point_3d_for_Ti = load_point_cloud_row_into_gaussian_point_3d(
                    #     pointcloud=pointcloud,
                    #     pointcloud_features=pointcloud_features,
                    #     point_id=point_id)
                    # translation_camera_for_Ti = ti.Vector(
                    #     [point_in_camera[point_offset, j] for j in ti.static(range(3))])
                    # point_q_camera_pointcloud_for_Ti = ti.Vector(
                    #     [q_camera_pointcloud[point_object_id[point_id], idx] for idx in ti.static(range(4))])
                    # point_t_camera_pointcloud_for_Ti = ti.Vector(
                    #     [t_camera_pointcloud[point_object_id[point_id], idx] for idx in ti.static(range(3))])
                    # T_camera_pointcloud_mat_for_Ti = transform_matrix_from_quaternion_and_translation(
                    #     q=point_q_camera_pointcloud_for_Ti,
                    #     t=point_t_camera_pointcloud_for_Ti,
                    # )
                    # d_Sigma_prime_d_q_point_cloud_camera_for_Ti = gaussian_point_3d_for_Ti.project_to_camera_position_covariance_jacobian(
                    #     T_camera_world=T_camera_pointcloud_mat_for_Ti,
                    #     projective_transform=camera_intrinsics_mat,
                    #     translation_camera=translation_camera_for_Ti,
                    # )
                    # point_grad_uv_cov_flat_for_Ti = ti.math.vec4(
                    #     point_uv_cov_grad[0, 0],
                    #     point_uv_cov_grad[0, 1],
                    #     point_uv_cov_grad[0, 1],
                    #     point_uv_cov_grad[1, 1],
                    # )
                    # grad_q_pointcloud_camera_for_Ti = point_grad_uv_cov_flat_for_Ti @ d_Sigma_prime_d_q_point_cloud_camera_for_Ti
                    # if first_loop_flag == 0.0:
                    #     pass
                    # elif first_loop_flag == 1.0:
                    #     d_Ti_d_q_now = - T_i * grad_q_pointcloud_camera_for_Ti * accumulated_muliplication
                    #     # for i in ti.static(range(4)):
                    #     #     ti.atomic_add(grad_q_pointcloud_camera_buffer[i], d_Ti_d_q_now[i])
                    #     accumulated_muliplication *= (1. - alpha)
                    # first_loop_flag = 1.0
    ...
    for idx in range(point_id_in_camera_list.shape[0]):
        ...
        translation_camera = ti.Vector([
            point_in_camera[idx, j] for j in ti.static(range(3))])
        d_uv_d_translation, d_uv_d_t_pointcloud_camera, d_uv_d_q_camera_pointcloud = gaussian_point_3d.project_to_camera_position_t_pointcloud_camera_jacobian(
            T_camera_world=T_camera_pointcloud_mat,
            projective_transform=camera_intrinsics_mat,
        ...
        # d_Sigma_prime_d_t_point_cloud_camera = gaussian_point_3d.project_to_camera_covariance_t_camera_pointcloud_jacobian(
        #     T_camera_world=T_camera_pointcloud_mat,
        #     projective_transform=camera_intrinsics_mat,
        #     translation_camera=translation_camera,
        # )
        d_Sigma_prime_d_q_point_cloud_camera, d_Sigma_prime_d_t_point_cloud_camera = gaussian_point_3d.project_to_camera_position_covariance_jacobian(
            T_camera_world=T_camera_pointcloud_mat,
            projective_transform=camera_intrinsics_mat,
            translation_camera=translation_camera,
            point_q_camera_pointcloud=point_q_camera_pointcloud,
        ...
        ray_direction = gaussian_point_3d.translation - ray_origin
        _, r_jacobian, g_jacobian, b_jacobian, ms_jacobian, r_direction_jacobian, g_direction_jacobian, b_direction_jacobian, ms_direction_jacobian = gaussian_point_3d.get_color_with_direction_jacobian_by_ray(
            ray_origin=ray_origin,
            ray_direction=ray_direction,
        )
        color_r_grad = point_grad_color[0] * r_jacobian
        ...
        t_pointcloud_camera_r_grad = -1.0 * point_grad_color[0] * r_direction_jacobian
        t_pointcloud_camera_g_grad = -1.0 * point_grad_color[1] * g_direction_jacobian
        t_pointcloud_camera_b_grad = -1.0 * point_grad_color[2] * b_direction_jacobian
        t_pointcloud_camera_rgb2gray_grad = 0.299 * t_pointcloud_camera_r_grad + 0.587 * t_pointcloud_camera_g_grad + 0.114 * t_pointcloud_camera_b_grad
        # t_pointcloud_camera_rgb2gray_grad = t_pointcloud_camera_r_grad + t_pointcloud_camera_g_grad + t_pointcloud_camera_b_grad
        # t_pointcloud_camera_ms_grad = -1.0 * color_ms_grad * ms_direction_jacobian
        translation_grad_t_camera_pointcloud = point_grad_uv @ d_uv_d_t_pointcloud_camera
        translation_grad_q_camera_pointcloud = point_grad_uv @ d_uv_d_q_camera_pointcloud
        uv_cov_grad_t_camera_pointcloud = point_grad_uv_cov_flat @ d_Sigma_prime_d_t_point_cloud_camera
        for i in ti.static(range(3)):
            # # the gradient from color
            ti.atomic_add(grad_t_pointcloud_camera_buffer[i], t_pointcloud_camera_rgb2gray_grad[i])
            # the gradient from projection matrix
            ti.atomic_add(grad_t_pointcloud_camera_buffer[i], translation_grad_t_camera_pointcloud[i])
            # the gradient from sigma'
            ti.atomic_add(grad_t_pointcloud_camera_buffer[i], uv_cov_grad_t_camera_pointcloud[i])
        translation_grad = point_grad_uv @ d_uv_d_translation
        ...
        grad_q_pointcloud_camera_to_uv_cov = point_grad_uv_cov_flat @ d_Sigma_prime_d_q_point_cloud_camera
        for i in ti.static(range(4)):
            # q_pointcloud_camera gradient from uv_cov
            ti.atomic_add(grad_q_pointcloud_camera_buffer[i], grad_q_pointcloud_camera_to_uv_cov[i])
            # q_pointcloud_camera gradient from uv
            ti.atomic_add(grad_q_pointcloud_camera_buffer[i], translation_grad_q_camera_pointcloud[i])
    ...
    for i in ti.static(range(3)):
        # grad_t_pointcloud_camera[0, i] = grad_t_pointcloud_camera_buffer[i] / point_id_in_camera_list.shape[0]
        grad_t_pointcloud_camera[0, i] = grad_t_pointcloud_camera_buffer[i]
    for i in ti.static(range(4)):
        # grad_q_pointcloud_camera[0, i] = grad_q_pointcloud_camera_buffer[i] / point_id_in_camera_list.shape[0]
        grad_q_pointcloud_camera[0, i] = grad_q_pointcloud_camera_buffer[i]
...
def gaussian_point_rasterisation_ms_only_backward(
        ...
        grad_q_pointcloud_camera_buffer: ti.types.ndarray(ti.f32, ndim=1),
        grad_t_pointcloud_camera_buffer: ti.types.ndarray(ti.f32, ndim=1),
        # tile_point_uv_ms: ti.types.ndarray(ti.f32, ndim=2),
        # tile_point_uv_conic_and_rescale_ms: ti.types.ndarray(ti.f32, ndim=2),
        # tile_point_color_ms: ti.types.ndarray(ti.f32, ndim=1),
        # tile_point_alpha_ms: ti.types.ndarray(ti.f32, ndim=1),
):
    camera_intrinsics_mat = ti.Matrix(
        [[camera_intrinsics[row, col] for col in ti.static(range(3))] for row in ti.static(range(3))])
    ti.loop_config(block_dim=(TILE_HEIGHT * TILE_WIDTH))
    ...
    for pixel_offset in ti.ndrange(camera_height * camera_width):
        ...
        # for i_, j_ in tile_point_uv:
        #     tile_point_uv[i_, j_] = 0.0
        # for i_, j_ in tile_point_uv_conic_and_rescale:
        #     tile_point_uv_conic_and_rescale[i_, j_] = 0.0
        # for i_ in tile_point_color:
        #     tile_point_color[i_] = 0.0
        # for i_ in tile_point_alpha:
        #     tile_point_alpha[i_] = 0.0
...
class GaussianPointCloudRasterisation(torch.nn.Module):
    ...
    class GaussianPointCloudRasterisationInput:
        ...
        # estimated q and t of multispectral modality
    ...
    def __init__(
        ...
        # for validation
        class _module_function_val_all(torch.autograd.Function):
            ...
            def forward(ctx,
                ...
                filter_point_in_camera(
                    pointcloud=pointcloud.contiguous(),
                    point_invalid_mask=point_invalid_mask.contiguous(),
                    point_object_id=point_object_id.contiguous(),
                    camera_intrinsics=camera_info.camera_intrinsics.contiguous(),
                    q_camera_pointcloud=q_camera_pointcloud.contiguous(),
                    t_camera_pointcloud=t_camera_pointcloud.contiguous(),
                    point_in_camera_mask=point_in_camera_mask.contiguous(),
                    near_plane=self.config.near_plane,
                    far_plane=self.config.far_plane,
                    camera_height=camera_info.camera_height,
                    camera_width=camera_info.camera_width,
                ...
                point_uv, point_alpha_after_activation, point_in_camera, point_uv_conic_and_rescale, point_color, point_radii = \
                    allocate_memory_for_filter_points(num_points_in_camera=num_points_in_camera,
                                                      pointcloud=pointcloud, color_channels=4)
                generate_point_attributes_in_camera_plane_all(
                    pointcloud=pointcloud.contiguous(),
                    pointcloud_features=pointcloud_features.contiguous(),
                    point_object_id=point_object_id.contiguous(),
                    camera_intrinsics=camera_info.camera_intrinsics.contiguous(),
                    point_id_list=point_id_in_camera_list.contiguous(),
                    q_camera_pointcloud=q_camera_pointcloud.contiguous(),
                    t_camera_pointcloud=t_camera_pointcloud.contiguous(),
                    point_uv=point_uv.contiguous(),
                    point_in_camera=point_in_camera.contiguous(),
                    point_uv_conic_and_rescale=point_uv_conic_and_rescale.contiguous(),
                    point_alpha_after_activation=point_alpha_after_activation.contiguous(),
                    point_color=point_color.contiguous(),
                    point_radii=point_radii.contiguous(),
                )
                ...
                rasterized_image, rasterized_depth, pixel_accumulated_alpha, pixel_offset_of_last_effective_point, pixel_valid_point_count = \
                    allocate_memory_for_rendered_images(
                        camera_width=camera_info.camera_width, camera_height=camera_info.camera_height,
                        pointcloud=pointcloud, rendered_channel=4,
                    )
        ...
        class _module_function_ms_only(torch.autograd.Function):
            ...
            def forward(ctx,
                ...
                q_camera_pointcloud, t_camera_pointcloud = inverse_SE3_qt_torch(q=q_pointcloud_camera,
                                                                                t=t_pointcloud_camera)
        ...
        class _module_function_pose_initialize(torch.autograd.Function):
            ...
            def forward(ctx,
                ...
                # # choose 4 non-coplanar points to solve EPnP
                # # redundant n points to avoid the situation that line and ellipse have no intersection
                # # or the four points coplanar
                # effective_EPnP_points_num = 0
                # redundant_points_num = 10
                # for keypoint_index in LoFTR_confidence_sort_down:
                #     if len(effective_src_keypoints_list) >= 4 + redundant_points_num:
                #         pass
                #     elif len(effective_src_keypoints_list) < 4 + redundant_points_num:
                #         effective_src_keypoints_list.append(src_pts[keypoint_index, :])
                #         effective_tgt_keypoints_list.append(dst_pts[keypoint_index, :])
        ...
        self._module_function_val_all = _module_function_val_all
    ...
    def forward(self, input_data: GaussianPointCloudRasterisationInput, current_train_stage: str, reference_ms_image=None):
        ...
        q_pointcloud_camera_multispectral = input_data.q_pointcloud_camera_multispectral
        t_pointcloud_camera_multispectral = input_data.t_pointcloud_camera_multispectral
        ...
        if current_train_stage == 's1' or current_train_stage == 'ending_RGB':
        ...
        if current_train_stage == 's2p1':
            return self._module_function_pose_only.apply(
                pointcloud,
                pointcloud_features,
                point_invalid_mask,
                point_object_id,
                q_pointcloud_camera_multispectral,
                t_pointcloud_camera_multispectral,
                camera_info,
                color_max_sh_band,
            )

        if current_train_stage == 'val_all':
            return self._module_function_val_all.apply(
                pointcloud,
                pointcloud_features,
                point_invalid_mask,
                point_object_id,
        ...
        if current_train_stage == 's2p2' or current_train_stage == 's2p3' or current_train_stage == 'ending_MS':
