...
class GaussianPointCloudTrainer:
    ...
    class TrainConfig(YAMLWizard):
        ...
        pose_learning_rate_decay_interval: int = 200
        position_learning_rate_decay_interval: int = 200
    ...
    def train(self):
        ...
        ms_train_data_loader = torch.utils.data.DataLoader(
            self.ms_train_dataset, batch_size=None, shuffle=True, pin_memory=True, num_workers=4)
        ...
        ms_train_data_loader_iter = cycle(ms_train_data_loader)
        ...
        for iteration in tqdm(range(self.config.num_iterations)):
            ...
            if iteration <= self.config.warmup_single_modality_iterations:
                ...
                image_gt, q_pointcloud_camera, t_pointcloud_camera, image_gt_multispectral, q_pointcloud_camera_multispectral, t_pointcloud_camera_multispectral, camera_info, camera_info_ms = next(
                ...
                # estimate_pose_optimizer.zero_grad()
                optimizer.zero_grad()
            ...
            if iteration == self.config.warmup_single_modality_iterations:
                # initial MS Gaussians
                optimizer.zero_grad()
                position_optimizer.zero_grad()
                print('Set the mean of RGB-SH as the initial value of MS-SH')
                # render initial image
                with torch.no_grad():
                    ms_initial = self.scene.point_cloud_features[:, 8 + 16:24 + 16]
                    self.scene.point_cloud_features[:, 56:56 + 16] = ms_initial
                del ms_initial

                # initialize pose buffer(use SO3)
                initial_ms_pose_r = np.zeros([len(ms_train_data_loader), 3])
                initial_ms_pose_t = np.zeros([len(ms_train_data_loader), 3])

                # pose estimate initialization
                with torch.no_grad():
                    scene_view_num = len(ms_train_data_loader)
                    initial_count = []
                    while len(initial_count) < scene_view_num:
                        image_gt, q_pointcloud_camera, t_pointcloud_camera, image_gt_multispectral, q_pointcloud_camera_multispectral, t_pointcloud_camera_multispectral, camera_info, camera_info_ms = next(
                            ms_train_data_loader_iter)
                        assert camera_info.camera_id == camera_info_ms.camera_id
                        if str(camera_info.camera_id) in initial_count:
                            ...
                            print(f"Initializing Image id:{camera_info.camera_id}")
                            ...
                            image_pred = image_pred.permute(2, 0, 1)
                            image_pred_name = 'before_BA_initialize_id_' + str(camera_info.camera_id) + '.png'
                            ...
                            gaussian_point_cloud_rasterisation_input_initial = GaussianPointCloudRasterisation.GaussianPointCloudRasterisationInput(
                                ...
                                q_pointcloud_camera_multispectral=q_pointcloud_camera_ms,
                                t_pointcloud_camera_multispectral=t_pointcloud_camera_ms,
                                color_max_sh_band=3,
                            )
                            ...
                            image_pred_name_initial = 'after_BA_initialize_id_' + str(camera_info.camera_id) + '.png'
                ...
                del _, gaussian_point_cloud_rasterisation_input, gaussian_point_cloud_rasterisation_input_ms, gaussian_point_cloud_rasterisation_input_initial
                ...
                estimate_pose_optimizer = torch.optim.Adam(self.pose_estimated_multispectral.parameters(),
                                                           lr=self.config.pose_learning_rate, betas=(0.9, 0.999))
                scheduler_ms_pose = torch.optim.lr_scheduler.ExponentialLR(
                    optimizer=estimate_pose_optimizer, gamma=self.config.position_learning_rate_decay_rate)
            ...
            if self.config.warmup_single_modality_iterations < iteration <= self.config.fine_bundle_adjustment_iteration:
                ...
                image_gt, q_pointcloud_camera, t_pointcloud_camera, image_gt_multispectral, q_pointcloud_camera_multispectral, t_pointcloud_camera_multispectral, camera_info, camera_info_ms = next(
                    ms_train_data_loader_iter)
                assert camera_info.camera_id == camera_info_ms.camera_id
                # calculate SE4 q and t from estimated SE3 transformer q & t and c2w of RGB
            ...
            if iteration > self.config.fine_bundle_adjustment_iteration:
                current_modality = modality_pool[modality_flag]
            ...
            if self.config.warmup_single_modality_iterations < iteration <= self.config.fine_bundle_adjustment_iteration and iteration % self.config.log_metrics_interval == 0:
            ...
            if self.config.warmup_single_modality_iterations < iteration <= self.config.fine_bundle_adjustment_iteration and iteration % 1000 == 0:
                self.store_current_pose_list(iteration, modal='ms', num_cameras=len(ms_train_data_loader))
                self.validation_for_cross_spectral(val_data_loader, iteration)
    ...
    def validation_for_cross_spectral(self, val_data_loader, iteration):
        ...
        with torch.no_grad():
            ...
            if mean_psnr_score_MS > self.best_psnr_score_ms:
                ...
                print('Best Metric RGB', self.best_psnr_score, self.best_ssim_score)
                print('Best Metric MS', self.best_psnr_score_ms, self.best_ssim_score_ms)
            if mean_psnr_score_RGB + mean_psnr_score_MS > self.best_psnr_score_store_parquet:
                self.best_psnr_score_store_parquet = mean_psnr_score_RGB + mean_psnr_score_MS
                self.scene.to_parquet(os.path.join(self.config.output_model_dir, f"best_scene.parquet"))
                print(f"<Best Parquet saved!> Metric RGB:{mean_psnr_score_RGB}, Metric MS:{mean_psnr_score_MS}")
    ...
    def store_current_pose_list(self, iteration, modal, num_cameras):
        ...
        with torch.no_grad():
            for camera_id in range(num_cameras):
                q_pointcloud_camera_ms, t_pointcloud_camera_ms, q_vec, t_vec = self.pose_estimated_multispectral(
                    camera_id)
    ...
    def validation(self, val_data_loader, iteration):
        ...
        del loss_RGB, rgb_image_gt, rgb_image_pred,
